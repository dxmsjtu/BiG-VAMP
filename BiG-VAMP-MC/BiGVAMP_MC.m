...
%   mu_z_post:   estimated Z matrix
    ...
    switch     params.prior_u
        case    {'Gauss'}
            u=randn(m,r);               mu_u_gauss_update = zeros(m,r);            gamma_u_gauss_update = ones(1,r);
            mu_u_ext_old = zeros(m,r);            gamma_u_ext_old = ones(1,r);
        case    {'Binary'}
            u = randn(m,r);                     mu_u_gauss_update = zeros(m,r);         gamma_u_gauss_update = ones(1,r);            
            mu_u_ext_old = zeros(m,r);            gamma_u_ext_old = ones(1,r);
        case    {'Bernoulli-Gauss'}
            sparsity = params.prior_u_paramsion.rho;            u=randn(m,r).*(rand(m,r)<sparsity);            mu_u_gauss_update = zeros(m,r);
            gamma_u_gauss_update = ones(1,r);                    mu_u_ext_old = zeros(m,r);           gamma_u_ext_old = ones(1,r);
        otherwise
    ...
    switch     params.prior_v
        case    {'Gauss'}
            v=randn(n,r);            mu_v_gauss_update = zeros(n,r);            gamma_v_gauss_update = ones(1,r);            
            mu_v_ext_old = zeros(n,r);            gamma_v_ext_old = ones(1,r);
        case    {'Bernoulli-Gauss'}
            v = randn(n,r);            mu_v_gauss_update = zeros(n,r);          gamma_v_gauss_update = ones(1,r);            
            mu_v_ext_old = zeros(n,r);            gamma_v_ext_old = ones(1,r);
        otherwise
    ...
    % initialize the mean of Z to the observation matrix, and the precision to 1
    Z = Y;    gamma_z_gauss_update = 1;    
    % initialize the estimation variables u, v, and z
    u_old = zeros(m,r);    v_old = zeros(n,r);    u_var = zeros(r,r);     v_var = zeros(r,r);
    mu_z_ext_old = Y;    gamma_z_ext_old = 1;    
    % initialize the bi-LMMSE variables
    A_u = zeros(r,r);    B_u = zeros(m,r);    A_v = zeros(r,r);    B_v = zeros(n,r);    
    % iteration variables
    damp=params.damping;    diff=1;    t=0;    
    % ensure the bi-LMMSE iterations start with high variance
    var_w = 100 * var_w_y;
    while ((diff>params.conv_criterion))&&(t<params.nb_iter)
        % account for the uncertainty of Z after a few message passing iterations to stabalize the algorithm
       ...
       % damp the bi-LMMSE variables
        A_u = (1-damp)*A_u_old + damp*A_u_new;        A_v = (1-damp)*A_v_old + damp*A_v_new;
        B_u = (1-damp)*B_u_old + damp*B_u_new;        B_v = (1-damp)*B_v_old + damp*B_v_new;
        ...
        switch     params.prior_u
            case    {'Gauss'}
                % do nothing
            case    {'Binary'}
                [u_est, mu_u_gauss_update, gamma_u_gauss_update] =  prior_binary(mu_u_ext,gamma_u_ext);
            case    {'Bernoulli-Gauss'}
                [u_est, mu_u_gauss_update, gamma_u_gauss_update] =  prior_bernoulli_gauss(mu_u_ext,gamma_u_ext, 1, params.prior_u_paramsion.rho);
        end        
