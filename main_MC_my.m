close all;  clear all;
addpath('functions/');addpath('BiG-VAMP-MC/');
% size of the matrices U (nxr) and V^T (mxr)
n=1000;m=500;r=10;   % n=1000;m=500;r=5;
% SNR level in dB
SNR_dB = 20;
% parameter object contaning the experiment's parameters
params = Parameters();params.damping = 0.5;
...
switch     params.prior_u
  ...
  case    {'Bernoulli-Gauss'}
    sparsity = params.prior_u_option.rho;    U = randn(n,r).*(rand(n,r)<sparsity);
  otherwise          
...
perm = randperm(m*n);
figure(1);stem(true_uv(perm(1:n)), 'b');hold on
stem(est_uv(perm(1:n)), '--r');legend('true', 'estimated');
title(['BiG-VAMP for matrix completion: ', 'SNR=', num2str(SNR_dB), 'dB',  ', n=', num2str(n), ', m=', num2str(m), ', r=', num2str(r)]);